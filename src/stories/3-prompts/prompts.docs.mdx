...
From this simple prompt we are able to convert data formats 🤯
...
const parser = StructuredOutputParser.fromNamesAndDescriptions({
  name: "users name",
  age: "users age",
  cars: : "users cars brands as a string",
});
```
...
const prompt = new PromptTemplate({
  template:
    `You are tasked with taking the users input and converting it to a JSON object.
    {format_instructions}
    {question}
    `,
...
console.log(prompt.format({ question: "My name is Josh Mabry and I'm 38 years old. I have a Pontiac GTO, a Pontiac Firebird, and a Pontiac Solstice. I also have a Ford Mustang, and a Ford Econoline"}))
```


#### Try It

Press the button and see the output of the language model

<Canvas of={PromptStories.StructuredOutputParserExample} />
...
const parser = StructuredOutputParser.fromZodSchema(
  z.object({
    ...
    cars: z.array(
      z.object({
        name: z.string().describe("the name of the car"),
...
const input = await prompt.format({
  question: `My name is Josh Mabry and I'm 38 years old. 
  I have a Pontiac GTO, a Pontiac Firebird, and a Pontiac Solstice. 
  I also have a Ford Mustang, and a Ford Econoline.`,
});
const response = await model.call(input);
// verify the output using the parse method
...
<Canvas of={PromptStories.ZodOutputParserExample} />
